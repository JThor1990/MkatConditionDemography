...
lapply(c("ggplot2", "grDevices", "ks", "mgcv", "patchwork", "sf", "tidyverse"),  FUN = library, character.only = TRUE)
...
forage_df$FocalDate <- as.Date(strptime(forage_df$FocalDate, format = "%d/%m/%Y"))

# Set up 3-way season 
forage_df <- forage_df %>%
  mutate(Season=case_when(FocalMonth%in%c(12,1,2,3,4) ~ "LateSummer",
...
tempMax <- forage_df %>% 
  ...
  unique()
tempMax %>% summarise(mean = mean(tempmax)) # seasonal mean temperature
...
pred_df <- tibble(Season = c("EarlySummer","LateSummer","Winter"),
  ...
  unnest(col = tempmax) %>%
  mutate(IndividID = 3, GroupRef = 7, FocalDuration = 1200, DiggingDur = 600)
...
forsucc_pred <- forsucc_pred %>% 
  mutate(Season = case_when(Season == "EarlySummer" ~ "Early summer", 
...
pred_df2 <- expand.grid(Season = c("LateSummer"), tempmax=32,
                                  numAvgHour=9, ndvi=seq(0.20,0.43, by = 0.005),
                        IndividID=3, GroupRef=7, FocalDuration = 1200)
